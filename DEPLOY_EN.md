...
### Issue 1: Container failed to start (port 8080)
...
**Cause:** `main.py` imported `from apscheduler.schedulers.asyncio import AsyncIOScheduler`, but `apscheduler` was not in `requirements.txt`, causing the container to fail on import at startup.
...
### Issue 2: Firestore connection failed (project does not exist)

**Error message:**
```
The requested resource is invalid: "projects/***" does not exist.
```

**Cause:** `FirestoreDB.__init__` called `_get_client()` at object creation time, attempting to connect to Firestore very early in Cloud Run's cold start before the environment and credentials were ready.

**Fix:** Switch to lazy initialization — only create the client on the first actual DB operation.

```python
class FirestoreDB:
    def __init__(self):
        self._db: Optional[firestore.AsyncClient] = None  # don't initialize here

    def _client(self) -> firestore.AsyncClient:
        if self._db is None:
            self._db = _get_client()  # create only on first use
        return self._db
```

---

### Issue 3: Firestore database not found (404)
...
**Cause:** The Firestore database from Step 4 had not been created yet, or had not finished provisioning.

**Fix:** Follow Step 4 to create the Firestore database (Native mode). No redeployment needed after creation — just refresh the page.

---

### Issue 4: UI displaying error messages in Chinese

**Cause:** Four `HTTPException detail` strings in `main.py` were hardcoded in Chinese and displayed directly in the app UI.
